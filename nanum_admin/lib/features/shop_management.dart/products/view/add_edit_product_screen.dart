...
Future<void> _loadCompleteProductData() async {
  ...
  void _updateTotalStock() {
    if (_variants.isNotEmpty) {
      final totalStock = _variants.fold(0, (sum, variant) => sum + variant.stockQuantity);
      setState(() {
  ...
  void _generateVariants() {
    ...
    setState(() {
      _variants = combinations.map((name) => ProductVariant(
        name: name,
        additionalPrice: 0,
  ...
  try {
    ...
    if (_isEditMode) {
      ...
      await viewModel.updateProductWithOptions(
        ...
        existingImageUrls: _existingImageUrls.isNotEmpty ? _existingImageUrls : null,
      );
    } else {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('상품이 성공적으로 저장되었습니다.')),
      );
            ...
            rows: _variants.asMap().entries.map((entry) {
              ...
              return DataRow(
                cells: [
                  ...
                  DataCell(
                    _buildVariantStockStatus(variant.stockQuantity),
                  ),
                ],
              );
            }).toList(),
