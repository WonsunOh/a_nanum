...
// 💡 관리자가 등록한 '공구 가능 상품' 목록을 가져오는 함수
  ...
  Future<void> createGroupBuyFromMaster({
    ...
    await _client.rpc('handle_create_group_buy_from_master', params: {
        'p_product_id': productId,
        'p_target_participants': targetParticipants,
        'p_initial_quantity': initialQuantity, 
        'p_deadline': DateFormat('yyyy-MM-dd').format(deadline),
      });
  }
...
Future<void> updateTargetQuantity({required int groupBuyId, required int newQuantity}) async {
 ...
 // 💡 내가 '개설한' 공구 목록을 가져오는 메소드
  ...
  // 💡 ID로 단일 공구 정보를 가져오는 메소드
  Future<GroupBuy?> getGroupBuyById(int id) async {
    try {
      final response = await _client
          .from('group_buys')
          .select('*, products(*)')
          .eq('id', id)
          .single();
      return GroupBuy.fromJson(response);
    } catch (e) {
      return null;
    }
  }
