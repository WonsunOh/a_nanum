...
// 바로구매 데이터 Provider
final directPurchaseProvider = StateProvider<DirectPurchaseData?>(
  (ref) => null,
);
...
class SelectedVariantItem {
  ...
  SelectedVariantItem({required this.variant, this.quantity = 1});
...
final selectedItemsProvider =
    StateNotifierProvider<SelectedItemsNotifier, List<SelectedVariantItem>>((
      ref,
    ) {
...
class _ProductDetailScreenState extends ConsumerState<ProductDetailScreen> {
  ...
  void _onOptionSelected(
    ...
    final productDetailAsync = ref.read(
  ...
  void _toggleFavorite() async {
    try {
      ...
      final resultMessage = await ref
          .read(wishlistToggleProvider.notifier)
  ...
  Widget build(BuildContext context) {
    ...
    final isWishlistedAsync = ref.watch(
      ...
      bottomNavigationBar: LayoutBuilder(
        builder: (context, constraints) {
          ...
          return productDetailAsync.when(
            data: (productDetailState) =>
                _buildBottomBar(context, productDetailState),
                ...
                child: Center(
                  child: GestureDetector(
                    onTap: () {
                      _safeSetState(() {
                        _selectedImageIndex =
                            _selectedImageIndex < allImages.length - 1
                            ? _selectedImageIndex + 1
                            : 0;
                      });
                    },
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildInfoSection(
                '배송비',
                '${NumberFormat.currency(locale: 'ko_KR', symbol: '₩').format(product.shippingFee)}',
              ),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          if (showDescription) _buildProductDetailTabs(product),
        ],
      ),
    );
  }
            ...
            child: Column(
              children: [
                for (int i = 0; i < optionGroups.length; i++)
                  Padding(
                    padding: EdgeInsets.only(
                      bottom: i < optionGroups.length - 1 ? 12 : 0,
                    ),
        ...
        children: [
          ...
          ...optionGroups.map((group) {
            ...
            return Padding(
              ...
              child: Row(
                children: [
                  Text('${group.name}: ', style: const TextStyle(fontSize: 13)),
                  Text(
                    selectedValue ?? '선택하세요',
                    style: TextStyle(
                      fontSize: 13,
  ...
  void _addVariantToList(ProductVariant variant) {
    ref
        ...
        .addItem(SelectedVariantItem(variant: variant));
  }
  ...
  Future<void> _addToCart(BuildContext context, ProductModel product) async {
    ...
    if (selectedItems.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('상품 옵션을 선택해주세요.')));
      return;
    }
  ...
  String _encodeDirectPurchaseData(
    List<SelectedVariantItem> selectedItems,
  ...
  void _handleCartError(Object error) {
    ...
    if (errorString.contains('rls') ||
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          _showLoginDialog(
            ref.read(productDetailProvider(widget.productId)).value!.product,
          );
        }
      });
      return;
    } else if (errorString.contains('stock') ||
